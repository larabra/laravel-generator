...
class ModelGenerator extends BaseGenerator
{
    ...
    private function fillTemplate($templateData)
    {
        ...
        $templateData = $this->fillFormAccessible($templateData);
    ...
    {
        if (
            !$this->commandData->getOption('softDelete')
            || !in_array('deleted_at', $this->commandData->hiddenFields)
        ) {
            $templateData = str_replace('$SOFT_DELETE_IMPORT$'.PHP_EOL, '', $templateData);
            $templateData = str_replace('$SOFT_DELETE$'.PHP_EOL, '', $templateData);
            $templateData = str_replace('$SOFT_DELETE_DATES$'.PHP_EOL, '', $templateData);
        } else {
            $templateData = str_replace(
                '$SOFT_DELETE_IMPORT$',
            ...
            $deletedAtTimestamp = config('larabra.laravel_generator.timestamps.deleted_at', 'deleted_at');
            $templateData = str_replace(
                '$SOFT_DELETE_DATES$',
    ...
    private function fillFormAccessible($templateData)
    {
        if (trait_exists(\Collective\Html\Eloquent\FormAccessible::class)) {
            $templateData = str_replace(
                '$FORM_ACCESSIBLE_IMPORT$',
                'use Collective\Html\Eloquent\FormAccessible;',
                $templateData
            );
            $templateData = str_replace(
                '$FORM_ACCESSIBLE$',
                infy_tab() . "use FormAccessible;\n",
                $templateData
            );
        }
        else{
            $templateData = str_replace('$FORM_ACCESSIBLE_IMPORT$'.PHP_EOL, '', $templateData);
            $templateData = str_replace('$FORM_ACCESSIBLE$'.PHP_EOL, '', $templateData);
        }
