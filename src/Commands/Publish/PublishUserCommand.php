...
class PublishUserCommand extends PublishBaseCommand
{
     ...
     * @var string
     */
    protected $name = 'larabra.publish:user';
    ...
    public function handle()
    {
        ...
        if (config('larabra.laravel_generator.options.repository_pattern')) {
    ...
    {
        $path = config('larabra.laravel_generator.path.routes', base_path('routes/web.php'));
    ...
    private function publishUserRepository()
    {
        ...
        $repositoryPath = config('larabra.laravel_generator.path.repository', app_path('Repositories/'));
    ...
    private function publishCreateUserRequest()
    {
        ...
        $requestPath = config('larabra.laravel_generator.path.request', app_path('Http/Requests/'));
    ...
    {
        $templateData = str_replace('$NAMESPACE_CONTROLLER$', config('larabra.laravel_generator.namespace.controller'), $templateData);

        $templateData = str_replace('$NAMESPACE_REQUEST$', config('larabra.laravel_generator.namespace.request'), $templateData);

        $templateData = str_replace('$NAMESPACE_REPOSITORY$', config('larabra.laravel_generator.namespace.repository'), $templateData);
